import { canonicalDecimal, divideRounded, formatFixed, MONEY_DIGITS, parseFixed, PRICE_DIGITS, QUANTITY_DIGITS } from './decimal';
...
export function buildTradeEvent(draft: TradeDraft, today: string, now = new Date().toISOString()): { event?: BuyEvent | SellEvent; error?: string } {
  ...
  if (quantity === undefined || quantity <= 0n) return { error: 'Quantity must be greater than zero (up to 8 decimal places).' };
  ...
  if (draft.mode === 'total') {
    ...
    if (price === undefined || price <= 0n) return { error: 'Price per unit must be greater than zero.' };
    if (fees < 0n) return { error: 'Fees must be zero or greater.' };
...
export function effectiveUnitPrice(totalAmount: unknown, quantity: unknown): string | undefined {
  ...
  return formatFixed(divideRounded(total * 10n ** BigInt(QUANTITY_DIGITS + PRICE_DIGITS - MONEY_DIGITS), units), PRICE_DIGITS);
}
